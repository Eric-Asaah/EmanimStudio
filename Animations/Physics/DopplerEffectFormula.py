# TITLE: Doppler Effect Formula Breakdown
# DESCRIPTION: Interactive breakdown of Doppler effect equation with term-by-term explanation and speed component analysis.
from manim import *
    ...
    def construct(self):
        ...
        for index, explanation_text in terms:
            ...
            highlight = SurroundingRectangle(
                target, color=YELLOW, fill_opacity=0.2)
            explanation = MathTex(explanation_text, font_size=36).next_to(
                formula, DOWN, buff=1)
            arrow = Arrow(explanation.get_top(),
                          target.get_bottom(), buff=0.15, color=YELLOW)
        ...
        self.play(ReplacementTransform(formula[3], Ratio), FadeOut(
        ...
        for speed_obj, explanation_text in explanations_speeds:
            explanation = Text(explanation_text, font_size=24).next_to(
                speed_obj, DOWN)
