...
# DESCRIPTION: Demonstration of the Collatz conjecture showing number sequence from 3 to 1 using 3n+1 for odd and n/2 for even numbers.
from manim import *
...
class CollatzConjecture(Scene):
    def construct(self):
        ...
        def process_block(current, operation_tex, result_tex):
            ...
            operation = MathTex(operation_tex).next_to(
                current, DOWN, buff=1).set_color_by_gradient(RED, YELLOW).scale(2)
            self.play(Write(operation))
            self.wait(1)

            # Result below operation
            result = MathTex(result_tex, color=BLUE).next_to(
                operation, DOWN, buff=1).scale(2)
        ...
        # --- Opening text ---
        opening = Tex("Pick any number you want").scale(
            1.2).set_color_by_gradient(RED, BLUE, GREEN)
        even = Tex("Even: Divide it by 2").scale(
            1.2).set_color_by_gradient(RED, BLUE, GREEN)
        odd = Tex("Odd: Multiply by 3 and add 1").scale(
            1.2).set_color_by_gradient(RED, BLUE, GREEN)
        check = Tex("Repeat this for the results").scale(
            1.2).set_color_by_gradient(RED, BLUE, GREEN)

        self.play(Write(opening))
        self.wait(2)
        self.play(ReplacementTransform(opening, even))
        self.wait(2)
        self.play(ReplacementTransform(even, odd))
        self.wait(2)
        self.play(ReplacementTransform(odd, check))
        self.wait(2)
        ...
        # Step 1: Odd → 3n + 1
        mult0 = MathTex("3", "(", "3", ")", "+", "1",
                        color=RED).next_to(three, DOWN, buff=1).scale(2.2)
        self.play(Write(mult0[0]), Write(mult0[1]), Write(
            mult0[3]), Write(mult0[4]), Write(mult0[5]))
        self.play(TransformFromCopy(three, mult0[2]))
        result0 = MathTex("10", color=BLUE).scale(
            2).next_to(mult0, DOWN, buff=1)
        ...
        self.play(ReplacementTransform(result_1, five))
        self.wait(1)

        # Step 1: Odd → 3n + 1
        mult1 = MathTex("3", "(", "5", ")", "+", "1",
                        color=RED).next_to(five, DOWN, buff=1).scale(2.2)
        self.play(Write(mult1[0]), Write(mult1[1]), Write(
            mult1[3]), Write(mult1[4]), Write(mult1[5]))
        self.play(TransformFromCopy(five, mult1[2]))
        result1 = MathTex("16", color=BLUE).scale(
            2).next_to(mult1, DOWN, buff=1)
        ...
        # --- Closing statement ---
        closing = Tex("No matter where you start, you always end at 1...").scale(
            0.9).set_color_by_gradient(BLUE, GREEN)
