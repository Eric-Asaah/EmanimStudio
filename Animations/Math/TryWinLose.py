# TITLE: Probability Tree: Try-Win-Lose with Highlights
# DESCRIPTION: Animated probability tree with strategic highlighting showing sequential attempts, outcomes, and retry decisions.
from manim import *
import numpy as np
...
class WinLose(MovingCameraScene):
    ...
    def construct(self):
        ...
        # Vertical space for branches (e.g., for WIN/LOSE separation)
        y_space = 1.5
        ...
        for frm, to in edges:
            ...
            if frm not in shown_nodes:
                ...
                # VOICEOVER: Describe the TRY situation
                highlight_mobject(
                    VGroup(created_nodes[frm], labels[frm]), 1.5 * speed)

                self.play(FadeIn(labels[frm]), run_time=0.7 * speed)
                # VOICEOVER: Explain what this attempt represents
                self.wait(1.5 * speed)
                shown_nodes.add(frm)
            ...
            self.play(
                self.camera.frame.animate.move_to(midpoint).set(
                    width=max(8, abs(points[to][0] - points[frm][0]) * 2)),
                run_time=0.8 * speed
            )
            ...
            # VOICEOVER: Introduce the possible outcome
            highlight_mobject(
                VGroup(created_nodes[to], labels[to]), 1.5 * speed)
            ...
            if "LOSE" in frm and "TRY" in to:
                ...
                highlight_mobject(
                    VGroup(created_nodes[frm], created_nodes[to], arrow), 2.0 * speed, color=BLUE)
                # VOICEOVER: Explain trying again after failure
                self.wait(1.5 * speed)

            # Extra wait for WIN outcomes
            if "WIN" in to:
                # VOICEOVER: Celebrate or explain success
                self.wait(1.5 * speed)
        ...
        for name in points:
            if name not in shown_nodes:
                ...
                # VOICEOVER: Explain any isolated points or final state.
                self.wait(1)
