...
try:
    from auto_repair import EmanimStudioRepair
...
class EmanimBackend:
    ...
    def get_categories(self):
        ...
        categories = self.renderer.get_animation_categories()

        # Convert to the expected format for the frontend
        # Filter out __pycache__ and other system directories
        filtered_categories = {}
        for cat_name, count in categories.items():
            if not cat_name.startswith('__') and cat_name != '__pycache__':
                filtered_categories[cat_name] = count
    ...
    def render_animation(self, category, filename, scene_name, quality="medium"):
        """Render an animation"""
        return self.renderer.render_animation(category, filename, scene_name, quality)
...
# Public API functions for batch file


def get_categories():
    return get_backend().get_categories()


def get_animations(category):
    return get_backend().get_animations(category)


def render_animation(category, filename, scene_name):
    return get_backend().render_animation(category, filename, scene_name)


def play_latest_video():
    return get_backend().play_latest_video()


def list_videos():
    return get_backend().list_videos()


def launch_gui():
    return get_backend().launch_gui()
...
def test_backend():
    ...
    for category_name, count in categories.items():
        print(f"   - {category_name} ({count} animations)")
